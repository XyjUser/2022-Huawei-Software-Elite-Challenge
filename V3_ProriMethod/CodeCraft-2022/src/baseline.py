...
class BaseLine(Settings):
    ...
    def initialize(self):
        for time_index in range(self.time_num):
            ...
            for site in self.site_keys:
                self.site_used_vol_list[time_index][site] = 0
                for i in range(len(self.stream_names_list[time_index])):
                    self.each_site_max_stream[time_index][site][i] = 0
    ...
    def get_suitable_site(self, st, time_index):
        ...
        for site in sites:
            ...
            if use_vol <= self.site_bandwidth[site]:
                ...
                total_cost = site_cost + center_cost * self.center_cost
    ...
    def get_target_info(self):
        site_total_used = [[self.site_used_vol_list[time_index][site]
                            for time_index in range(self.time_num)] for site in self.site_keys]
        site_cost_info = [0 for _ in self.site_keys]
        site_max_vol = [0 for _ in self.site_keys]
        for site in self.site_keys:
            obj = sorted(range(self.time_num), key=lambda k: site_total_used[site][k])[self.object_position]
            site_cost_info[site] = (site_total_used[site][obj], obj)
            site_max_vol[site] = site_total_used[site][-1]
        return site_cost_info, site_max_vol
