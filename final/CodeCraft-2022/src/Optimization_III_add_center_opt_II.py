...
class Opt(BaseLine):
    ...
    def get_available_site(self, st, time_index):
        ...
        for site in sites:
            ...
            # if change_vol_center == 0 and change_vol_site == 0:
            #     return False, None, 0
            if change_vol <= 0:
    ...
    def site_change_vol(self, site_from, site_to, st, time_index):
        ...
        if time_index < site_from_time_95:
            ...
            # change_vol_from = self.calc_cache(site_from, st.vol, time_index, site_from_time_95)
    ...
    def opt_center_cost(self):
        ...
        for st in self.client_demands[time_index]:
            ...
            if flag:
                ...
                print(f"time_index: {time_index} move site_from: {site_from}  site_to: {to_site}  move stream:{st} "
                      f"change_vol: {int(change_vol)} center_score: {self.check_center_score()}")
                # self.check_center_record()
                # print(change_vol)
                return change_vol
        return 0
    ...
    def opt_site_cost_once(self, site):
        ...
        for st in self.client_demands[time_index]:
            ...
            if flag:
                self.move_stream(st, st.site, to_site, time_index)
                print(
                    f"time_index: {time_index} move site_from: {site}  site_to: {to_site}  move stream:{st} "
                    f"change_vol: {int(change_vol)} center_score: {self.check_center_score()} site_score: {self.check_site_score()}")
                # self.check_center_record()
                return change_vol
        return 0
    ...
    def check_center_record(self):
        center_record_list = [[[[0,0]for _ in self.stream_names_list[time_index]]for _ in self.site_keys]for time_index in self.time_index_tuple]
        for time_index in self.time_index_tuple:
            for st in self.client_demands[time_index]:
                vol = st.vol
                site = st.site
                if vol >= center_record_list[time_index][site][st.stream][0]:
                    center_record_list[time_index][site][st.stream][1] = center_record_list[time_index][site][st.stream][0]
                    center_record_list[time_index][site][st.stream][0] = vol
                else:
                    if vol >= center_record_list[time_index][site][st.stream][1]:
                        center_record_list[time_index][site][st.stream][1] = vol

        for time_index in self.time_index_tuple:
            for site in self.site_keys:
                for stream in range(len(self.stream_names_list[time_index])):
                    maxm = self.each_site_max_stream[time_index][site][stream]
                    minm = self.each_site_2nd_stream[time_index][site][stream]
                    if center_record_list[time_index][site][stream][0] != maxm or center_record_list[time_index][site][stream][1] != minm:
                        print(False)
                        return

    def check_center_score(self):
        center_score_time = [0 for _ in self.time_index_tuple]
        for site in self.site_keys:
            for time_index in self.time_index_tuple:
                # self.site_vol_list[site][time_index] = self.site_used_vol_list[time_index][site]
        ...
        center_score = round(center_score_time[center_record_time] * self.center_cost, 2)
        # print(center_score)
        return center_score

    def check_site_score(self):
        sum_score = 0
        for site in self.site_keys:
            arr = []
            for time_index in self.time_index_tuple:
    ...
    def run(self):
        ...
        self.opt_init()
        last_site_score, last_center_score, last_all_score = self.get_score()
        # change_vol = 0
        # for _ in range(20):
        #     change_vol += self.opt_center_cost()
        change_vol = self.opt_site_cost()
        new_site_score, new_center_score, new_all_score = self.get_score()
        print(f'{change_vol} {new_all_score - last_all_score}')
        self.judge()
        self.analyze()
        self.write_to_txt()
