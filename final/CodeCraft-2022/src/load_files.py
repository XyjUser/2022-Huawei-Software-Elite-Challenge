...
class Stream:
    def __init__(self, client, stream, vol, site=-1):
        self.client, self.site, self.stream, self.vol = client, site, stream, vol
        self.id = self.client * 100 + self.stream

    def __str__(self):
        return f"client_id:{self.client} stream_id:{self.stream} vol:{self.vol}"
...
def add_to_time_map(data, client_names, time_map, client_id_map, stream_names):
    ...
    for i, client in enumerate(client_names):
        ...
        if int(nums[i]) == 0:
            continue
        time_map.append(Stream(client_id_map[client], len(stream_names) - 1, int(nums[i])))
