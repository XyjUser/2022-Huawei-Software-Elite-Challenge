...
class BaseLine(Settings):
    def __init__(self):
        ...
        # self.threshold_value = [self.site_bandwidth[site] for site in self.site_keys]
        self.threshold_value = [self.site_bandwidth[site] for site in self.site_keys]
        ...
        self.time_labeled_sites = [[] for _ in self.time_index_tuple]
    ...
    def handle_demand(self):
        ...
        for site in sites:
            ...
            self.maximum_handle_site(site[0], time_list)

    def maximum_handle_site(self, site, time_list):
        clients = self.get_leisure_client(site)
        c_01, c_05 = int(self.site_bandwidth[site] * 0.01), int(self.site_bandwidth[site] * 0.05)
        for i, time_index in enumerate(time_list):
            self.time_labeled_sites[time_index].append(site)
            if time_index == 0:
                bandwidth = self.site_bandwidth[site]
            else:
                if time_index - 1 == time_list[i - 1]:
                    bandwidth = self.site_bandwidth[site] - c_05
                else:
                    bandwidth = self.site_bandwidth[site] - c_01
    ...
    def choose_time_index(self, site):
        time_list = heapq.nlargest(self.max_pre_handle_num, zip(self.time_index_tuple, self.site_sum_list[site]),
                                   key=lambda x: x[1])
        time_map = {}
        num = 0
        for time_index, vol in time_list:
            if time_index in time_map:
                continue
            time_map[time_index] = None
            num += 1
            if num == self.max_pre_handle_num:
                break
            cache = int(self.site_bandwidth[site] * 0.05) if vol > self.site_bandwidth[site] else int(vol * 0.05)
            t = time_index + 1
            while t < self.time_num and cache >= self.threshold_value[site] and t not in time_map:
                time_map[t] = None
                num += 1
                if num == self.max_pre_handle_num:
                    break
                vol = self.site_sum_list[site][t] + cache
                cache = int(self.site_bandwidth[site] * 0.05) if vol > self.site_bandwidth[site] else int(vol * 0.05)
                t += 1
                break
    ...
    def get_suitable_site(self, time_index, st, sties_map):
        ...
        if under_threshold_site:
            for site, vol in under_threshold_site:
                cost = st.vol * self.center_cost if st.vol > self.each_site_max_stream[time_index][site][
                    st.stream] else 0
                score = self.get_site_cost(vol, site) if site not in sties_map else 0
                if cost == min_cost:
        ...
        for site, vol in above_threshold_site:
            center_score = st.vol * self.center_cost if st.vol > self.each_site_max_stream[time_index][site][
                st.stream] else 0
            site_cost = self.get_site_cost(vol, site) if site not in sties_map else 0
